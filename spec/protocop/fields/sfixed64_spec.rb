...
describe Protocop::Fields::Sfixed64 do
  ...
  describe "#encode" do
    ...
    context "when the field is not repeated" do
      ...
      let(:field) do
        described_class.new(:sfixed64, :test, 1)
      end
    ...
    context "when the field is repeated" do
      ...
      context "when the field is not packed" do
        ...
        let(:field) do
          described_class.new(:sfixed64, :test, 1, repeated: true)
        end
        ...
        it "encodes the field, type and integer" do
          expect(buffer.bytes).to eq(
            "\t,\x01\x00\x00\x00\x00\x00\x00\t\x02\x00\x00\x00\x00\x00\x00\x00"
          )
        end
      ...
      context "when the field is packed" do
        ...
        let(:field) do
          described_class.new(:sfixed64, :test, 1, repeated: true, packed: true)
        end
        ...
        it "encodes the field, type and integer" do
          expect(buffer.bytes).to eq(
            "\n\x10,\x01\x00\x00\x00\x00\x00\x00\x02\x00\x00\x00\x00\x00\x00\x00"
          )
        end
